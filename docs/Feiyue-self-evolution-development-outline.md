...
- static report export：`export_static_runs_report(project_root, output_dir)` / `feiyue-runs-export` 将 dashboard 与 run detail 产出为离线 HTML 文件（`index.html` + `runs/<task_id>.html`），用于无本地 server 的 fallback/human review。
...
- `feiyue-runs-export --root <project> --out <dir>` → writes offline `index.html` and per-run detail pages for serverless review.
