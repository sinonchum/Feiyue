...
- **8G post-merge no-deploy handoff**：added `PostMergeVerificationHandoff` and `post-merge-handoff`. Smoke `wave8-8g-pr3-post-merge-no-deploy-handoff` recorded post-merge verification status `handoff_ready`, `post_merge_verification_passed`, `no_deploy_release_handoff_ready`, release_handoff_ready `true`, local_test_baseline `695 passed`, CI run `27510205254`, deploy_performed `false`, and production_mutated `false`.
...
- **Wave9-3 exact real multi-worker dry-run authorization**：added `Wave9TaskPackAuthorization`, `Wave9AuthorizationCheck`, and `approve-wave9-real-multi-worker-run`. Authorization `wave9-3-real-multi-worker-dry-run-authorization` binds action `execute_wave9_real_multi_worker_dry_run`, task-pack hash `f8ab0e3682e25a033c1f19c603dbe82f45c2f92cd42558feac3e3acfcf09aecb`, worker profiles, verifier commands, merge strategy, provider_call_count `0`, dry_run_only: true, promotion_attempted: false, global_hermes_config_mutated: false, and production_mutated `false`.
- **Wave9-4 real multi-worker dry-run execution**：added `Wave9TaskPackExecutor` and `run-approved-wave9-real-multi-worker-dry-run`. Real smoke `wave9-4-real-multi-worker-dry-run` consumed the Wave9-3 authorization with `feiyue-mid-deepseek-pro` + `feiyue-strong-gpt55`, provider_call_count `2`, conflict_files `[]`, reason `combined_verifier_passed`, dry_run_only: true, promotion_attempted: false, external_pr_created: false, merge_performed: false, deploy_performed: false, global_hermes_config_mutated: false, production_mutated `false`, and source_repo_clean: true.
- **Wave9-5 local PR-plan bridge**：added `Wave9LocalPRPlan` and `wave9-local-pr-plan`. Smoke `wave9-5-local-pr-plan` converted verified execution `wave9-4-real-multi-worker-dry-run` into provider-free local PR-plan evidence with provider_call_count `0`, changed files for `wave9_marker.py` and `test_wave9_marker.py`, reason `local_pr_plan_only`, dry_run_only: true, promotion_attempted: false, external_pr_created: false, merge_performed: false, deploy_performed: false, global_hermes_config_mutated: false, production_mutated `false`, and source_repo_clean: true.
- **Wave9-6 local branch materialization**：added `Wave9LocalBranchMaterialization`, `approve-wave9-local-pr-plan-materialization`, and `materialize-wave9-local-pr-plan`. Smoke `wave9-6-local-branch-materialization` materialized `wave9-5-local-pr-plan` into local branch `wave9/real-multi-worker-marker` with provider_call_count `0`, local_branch_created: true, reason `local_branch_materialized` + `local_verifier_passed`, branch_pushed: false, external_pr_created: false, merge_performed: false, deploy_performed: false, promotion_attempted: false, production_mutated `false`, and source_repo_clean: true.
- **Wave9-7 local branch commit evidence**：added `Wave9LocalBranchCommit`, `approve-wave9-local-branch-commit`, and `commit-wave9-local-branch`. Smoke `wave9-7-local-branch-commit` committed verified materialization `wave9-6-local-branch-materialization` locally on `wave9/real-multi-worker-marker` with provider_call_count `0`, local_commit_created: true, local commit SHA `1a95bfc4c0578cef566b515da31047df1eda595e`, reason `local_verifier_passed` + `local_branch_commit_created`, branch_pushed: false, external_pr_created: false, merge_performed: false, deploy_performed: false, promotion_attempted: false, and production_mutated `false`.
- **Wave9-8 Draft PR gate**：added `Wave9DraftPRApproval`, `Wave9DraftPR`, `approve-wave9-draft-pr`, and `create-wave9-draft-pr`. The gate consumes Wave9-7 local commit evidence, exact-binds commit hash, branch, target branch, and changed files, and permits only Draft PR creation with auto_merge_enabled: false, merge_performed: false, deploy_performed: false, and production_mutated `false`.
- **Wave9-9/10 PR review and capability ingestion**：added provider-free `Wave9PRSemanticReview`, `Wave9CapabilityIngestion`, `wave9-pr-semantic-review`, and `wave9-capability-ingest`. The evidence path records provider_call_count `0`, mutates_state: false, routing_table_mutated: false, merge_performed: false, deploy_performed: false, and production_mutated `false`.
- **Wave9-11/15 PR readiness, merge, and no-deploy handoff**：PR #4 (`https://github.com/sinonchum/Feiyue/pull/4`) was marked ready-for-review, checks passed, then exact-merged by squash. Merge commit `78ae64c6751741b9a0924fd8ff8457ba54c690ee`; post-merge local verification recorded `695 passed`; handoff `wave9-15-pr4-post-merge-no-deploy-handoff` has release_handoff_ready true, deploy_performed: false, auto_merge_enabled: false, and production_mutated `false`.
- **Wave10-1 through Wave13-1 execution batch**：executed on 2026-06-15 as evidence-first local operations. Wave10-1 `wave10-1-longitudinal-mini-program-20260615` recorded `teacher_call_rate_delta: -1.0`, retry_count_delta `-2`, repeat_error_count_delta `-2`, `provider_call_count: 0`, `dry_run_only: true`, and `production_mutated: false`. Wave10-2 `wave10-2-asset-reuse-smoke-20260615` loaded the promoted lesson `.hermes/lessons/wave5-teacher-reduction-medium-20260614.md` and recorded verified after-metrics with no teacher call. Wave10-3 refreshed capability-history to 127 local records plus longitudinal-gain without routing/global-config mutation. Wave11-1 `review-inbox --format json` reported 63 read-only review items with `mutates_state: false`; Wave11-2 `wave11-2-evidence-explorer-20260615` exported, verified, bundled, extracted, and re-verified the static evidence explorer including `review-inbox/index.html`. Wave12-1 `wave12-1-creative-feedback-20260615` captured deferred creative-role feedback with provider_call_count `0`. Wave13-1 `wave13-1-deploy-rollback-readiness-20260615` verified merge/rollback/deploy readiness design only with `approval_applies: true`, `merge_performed: false`, `auto_merge_enabled: false`, `deploy_performed: false`, and `production_mutated: false`; a real deployment remains blocked until a target environment and exact approval exist.
- **Wave14-1 through Wave14-4 closure batch**：added `Wave14ClosureRunner` and executed `wave14-closure-20260615`. The closure evidence covers Wave14-1 longitudinal 3-task boundary with `longitudinal_task_count: 3`, Wave14-2 operator cockpit v1 with `operator_cockpit_v1_ready: true`, Wave14-3 routing learning apply loop with `routing_learning_apply_loop_ready: true` and `routing_table_mutated: false`, and Wave14-4 local release artifact selection with `deploy_target_selected: local-release-artifact`, `requires_exact_deploy_approval: true`, `deploy_performed: false`, and `production_mutated: false`.
- **Wave14 real-environment 1-4 authorized run**：after explicit user authorization (`我授权，真实环境跑一轮1-4`), executed `wave14-real-env-1to4-20260615`. Wave14-1 made one real `feiyue-mid-deepseek-pro` Hermes profile subprocess call against a clean local toy repo and verified the patch with `provider_call_count: 1`, `status: verified`, dry_run_only true, no promotion, and no global config mutation. Wave14-2 exported `.hermes/static-run-reports/wave14-real-env-1to4-20260615-operator-cockpit.zip` with `static_export_all_ok: true`. Wave14-3 generated `wave14-real-env-1to4-20260615-routing-learning-proposal` but kept a **safe routing apply hold** because the proposal includes stale/non-installed `steady-4c` alongside `feiyue-mid-deepseek-pro`; routing_table_mutated remained false. Wave14-4 verified `wave14-real-env-1to4-20260615-local-release-artifact-readiness-v2` with local release artifact readiness; `deploy_performed: false` and `production_mutated: false` because no external production target was supplied.
- **Wave14 filtered Routing Apply**：executed after user instruction to run Routing Apply before deployment discussion. Filtered proposal `wave14-real-env-1to4-20260615-routing-learning-filtered` `steady-4c excluded`, applied only `feiyue-mid-deepseek-pro`, and wrote apply evidence with `status: applied`, `applied_profiles: feiyue-mid-deepseek-pro`, and `routing_table_mutated: false` because the worker route already used the same profile. No deploy was run: `deploy_performed: false`, `production_mutated: false`.
- **Wave14 Deployment 1 GitHub Release Artifact**：after explicit approval (`同意Deployment 1`), published release `wave14-real-env-20260615` at https://github.com/sinonchum/Feiyue/releases/tag/wave14-real-env-20260615 with asset `wave14-real-env-1to4-20260615-operator-cockpit.zip`. The asset SHA256 is `b6913a8cf30c4e1f2bfcacfcc421717744edf0a972ef596f3f849c3e4bf8301e`; post-download verification recorded `post_download_manifest_verify: STATIC_REPORT_VERIFY_OK checked_files=3`. Deployment scope was `deploy_target: github_release_artifact`, `deploy_performed: true`, no service started, no external API deployed, and `production_mutated: false`.
- **Wave14 Deployment 2 GitHub Pages Static Cockpit**：after user selected option A, created standalone public repo `sinonchum/Feiyue-operator-cockpit` and published the cockpit at https://sinonchum.github.io/Feiyue-operator-cockpit/wave14-real-env-20260615/ without publicizing the private Feiyue source repo. Recorded `public_pages_commit: 964c138914c567d1b0f0102597df1bd23e3a28d4`; verified `index.html`, `manifest.json`, `assets/index.html`, and `review-inbox/index.html` returned HTTP 200. Deployment scope: `deploy_target: github_pages_static_cockpit_public_repo`, `deploy_performed: true`, `github_pages_config_mutated: true`, `private_source_repo_publicized: false`, `external_api_deployed: false`, `provider_call_count: 0`, and `production_mutated: false`.
...
- New targeted contracts cover review UI disabled actions, CLI reference generation, provider-free semantic review evidence, creative acceptance/taste metrics, Wave10-1 through Wave13-1 execution-batch status sync, and Wave14 closure evidence, and Wave14 real-environment 1-4 authorized-run docs, and Deployment 1 GitHub Release artifact docs, and Deployment 2 public GitHub Pages cockpit docs.
