...
- `feiyue-runs wave9-pr-semantic-review --pr-id <id> --review-id <id> --diff-path <path>` — Run provider-free semantic/safety review over a Wave9 Draft PR diff artifact; no PR mutation, merge, or deploy.
- `feiyue-runs wave9-capability-ingest --ingestion-id <id> --commit-id <id> --pr-id <id> --review-id <id>` — Ingest Wave9 commit, Draft PR, and semantic-review evidence into provider-free capability/evaluation evidence; routing table remains unchanged.
