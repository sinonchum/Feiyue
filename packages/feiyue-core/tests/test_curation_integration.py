...
from feiyue_core.curation import CuratorInput, ProposalPatch, ProposalStatus, build_distillation_proposal
...
def test_curation_connects_workflow_assets_to_review_required_distillation_proposal(tmp_path) -> None:
    ...
    curator_input = CuratorInput.from_assets(
        ...
        bug_dossier=dossier,
        teacher_guidance="Keep the proposal review-first and preserve provenance IDs.",
    ...
    assert "proposal-review-required" in rendered_curator_input
    ...
    proposal = build_distillation_proposal(
        ...
        patches=[
            ProposalPatch(
                ...
                source_ids=[curator_input.input_id, lesson.lesson_id],
            ),
            ProposalPatch(
                ...
                source_ids=[curator_input.input_id, regression_check.check_id],
            ),
            ProposalPatch(
                ...
                source_ids=[curator_input.input_id, "curator"],
            ),
        ],
    )
    ...
    assert "- Asset Type: lesson" in rendered_proposal
    assert "- Asset Type: regression_eval" in rendered_proposal
    assert "- Asset Type: routing_rule" in rendered_proposal
    assert "curator-m6-demo-task" in rendered_proposal
