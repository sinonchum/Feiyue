...
def test_command_runner_captures_success_output(tmp_path) -> None:
    ...
    result = runner.run([sys.executable, "-c", "print('hello feiyue')"], cwd=tmp_path)
...
def test_command_runner_marks_nonzero_exit_as_failed(tmp_path) -> None:
    ...
    result = runner.run([sys.executable, "-c", "import sys; print('bad'); sys.exit(3)"], cwd=tmp_path)
...
def test_command_runner_times_out_without_hanging(tmp_path) -> None:
    ...
    result = runner.run([sys.executable, "-c", "import time; time.sleep(5)"], cwd=tmp_path)
