...
def test_release_checklist_captures_m14_gates_and_authorization_boundaries() -> None:
    ...
    assert "Current baseline: 703 passed" in content
...
def test_status_docs_capture_wave4_real_profile_benchmark_checkpoint() -> None:
    ...
    for content in (readme, outline):
        assert "Current verified baseline: `703 passed`" in content
...
def test_status_docs_capture_8e_nondraft_merge_readiness_refresh() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "RefreshedMergeReadinessEvidence" in content
        assert "refresh-merge-readiness" in content
        assert "wave8-8e-pr3-nondraft-merge-readiness-refresh" in content
        assert "pr_non_draft_checks_passed_merge_readiness_refreshed" in content
        assert "status `ready_for_human_merge_review`" in content or "ready_for_human_merge_review" in content
        assert "isDraft `false`" in content or "isDraft false" in content
        assert "checks_passed `true`" in content or "checks_passed true" in content
        assert "merge_performed `false`" in content or "merge_performed false" in content
        assert "auto_merge_enabled `false`" in content or "auto_merge_enabled false" in content
        assert "deploy_performed `false`" in content or "deploy_performed false" in content
        assert "production_mutated `false`" in content or "production_mutated false" in content

    assert "refresh-merge-readiness" in cli_reference



def test_status_docs_capture_8f1_pre_merge_final_audit() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "PreMergeFinalAudit" in content
        assert "pre-merge-final-audit" in content
        assert "wave8-8f1-pr3-pre-merge-final-audit" in content
        assert "pre_merge_final_audit_passed_approval_request_ready" in content
        assert "approval_requested" in content
        assert "merge_approval_request_ready `true`" in content or "merge_approval_request_ready true" in content
        assert "head SHA `3f6537124c4bf353daf09cf2384d8eceb30b6a86`" in content or "3f6537124c4bf353daf09cf2384d8eceb30b6a86" in content
        assert "merge_performed `false`" in content or "merge_performed false" in content
        assert "auto_merge_enabled `false`" in content or "auto_merge_enabled false" in content
        assert "deploy_performed `false`" in content or "deploy_performed false" in content
        assert "production_mutated `false`" in content or "production_mutated false" in content

    assert "pre-merge-final-audit" in cli_reference



def test_status_docs_capture_8f2_real_merge_gate() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "RealMergeExecutionApproval" in content
        assert "RealMergeExecutionEvidence" in content
        assert "approve-real-merge" in content
        assert "execute-approved-real-merge" in content
        assert "execute_real_github_merge" in content
        assert "wave8-8f2-pr3-real-merge-approval" in content
        assert "real_merge_execution_approval_applies" in content
        assert "status `merged`" in content or "status merged" in content
        assert "github_pr_merged" in content
        assert "merge commit `7615eff150594a6d42c89ba5b309921d9d712ebb`" in content or "7615eff150594a6d42c89ba5b309921d9d712ebb" in content
        assert "merge_method `squash`" in content or "merge_method squash" in content
        assert "auto_merge_enabled `false`" in content or "auto_merge_enabled false" in content
        assert "deploy_performed `false`" in content or "deploy_performed false" in content
        assert "production_mutated `false`" in content or "production_mutated false" in content

    assert "approve-real-merge" in cli_reference
    assert "execute-approved-real-merge" in cli_reference



def test_status_docs_capture_8g_post_merge_no_deploy_handoff() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "PostMergeVerificationHandoff" in content
        assert "post-merge-handoff" in content
        assert "wave8-8g-pr3-post-merge-no-deploy-handoff" in content
        assert "post_merge_verification_passed" in content
        assert "no_deploy_release_handoff_ready" in content
        assert "release_handoff_ready `true`" in content or "release_handoff_ready true" in content
        assert "local_test_baseline `695 passed`" in content or "695 passed" in content
        assert "deploy_performed `false`" in content or "deploy_performed false" in content
        assert "production_mutated `false`" in content or "production_mutated false" in content

    assert "post-merge-handoff" in cli_reference



def test_status_docs_capture_8h_wave9_planning() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    plan = REPO_ROOT / "docs" / "plans" / "wave9-real-multi-worker-project-slice-plan.md"
    plan_text = plan.read_text(encoding="utf-8")

    for content in (readme, outline, plan_text):
        assert "8H" in content
        assert "Wave 9" in content or "Wave9" in content
        assert "real multi-worker project slice" in content
        assert "Wave9-2" in content
        assert "dry_run_only: true" in content
        assert "promotion_attempted: false" in content
        assert "global_hermes_config_mutated: false" in content
        assert "No deploy" in content or "no-deploy" in content or "deploy_performed `false`" in content
        assert "production_mutated" in content

    assert "docs/plans/wave9-real-multi-worker-project-slice-plan.md" in readme
    assert "wave9-real-multi-worker-project-slice-plan.md" in outline



def test_status_docs_capture_wave9_2_task_pack() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")
    plan = REPO_ROOT / "docs" / "plans" / "wave9-real-multi-worker-project-slice-plan.md"
    ...
    for content in (readme, outline):
        assert "Wave9-2" in content
        assert "Wave9TaskPack" in content
        assert "wave9-task-pack" in content
        assert "wave9-2-real-multi-worker-task-pack" in content
        assert "provider_call_count `0`" in content or "provider_call_count 0" in content
        assert "dry_run_only: true" in content
        assert "promotion_attempted: false" in content
        assert "global_hermes_config_mutated: false" in content
        assert "production_mutated `false`" in content or "production_mutated: false" in content

    assert "Wave9-2: Real Multi-worker Task Pack" in plan_text
    assert "wave9-task-pack" in cli_reference



def test_status_docs_capture_wave9_3_exact_authorization() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "Wave9-3" in content
        assert "Wave9TaskPackAuthorization" in content
        assert "Wave9AuthorizationCheck" in content
        assert "approve-wave9-real-multi-worker-run" in content
        assert "wave9-3-real-multi-worker-dry-run-authorization" in content
        assert "execute_wave9_real_multi_worker_dry_run" in content
        assert "f8ab0e3682e25a033c1f19c603dbe82f45c2f92cd42558feac3e3acfcf09aecb" in content
        assert "provider_call_count `0`" in content or "provider_call_count 0" in content
        assert "dry_run_only: true" in content
        assert "promotion_attempted: false" in content
        assert "global_hermes_config_mutated: false" in content
    ...
    assert "approve-wave9-real-multi-worker-run" in cli_reference



def test_status_docs_capture_wave9_4_real_multi_worker_dry_run() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "Wave9-4" in content
        assert "Wave9TaskPackExecutor" in content
        assert "run-approved-wave9-real-multi-worker-dry-run" in content
        assert "wave9-4-real-multi-worker-dry-run" in content
        assert "combined_verifier_passed" in content
        assert "provider_call_count `2`" in content or "provider_call_count 2" in content
        assert "dry_run_only: true" in content
        assert "promotion_attempted: false" in content
        assert "external_pr_created: false" in content
        assert "merge_performed: false" in content
        assert "deploy_performed: false" in content
        assert "global_hermes_config_mutated: false" in content
        assert "production_mutated `false`" in content or "production_mutated: false" in content
        assert "source_repo_clean: true" in content

    assert "run-approved-wave9-real-multi-worker-dry-run" in cli_reference



def test_status_docs_capture_wave9_5_local_pr_plan() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "Wave9-5" in content
        assert "Wave9LocalPRPlan" in content
        assert "wave9-local-pr-plan" in content
        assert "wave9-5-local-pr-plan" in content
        assert "wave9-4-real-multi-worker-dry-run" in content
        assert "local_pr_plan_only" in content
        assert "provider_call_count `0`" in content or "provider_call_count 0" in content
        assert "dry_run_only: true" in content
        assert "promotion_attempted: false" in content
        assert "external_pr_created: false" in content
    ...
    assert "wave9-local-pr-plan" in cli_reference



def test_status_docs_capture_wave9_6_local_branch_materialization() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "Wave9-6" in content
        assert "Wave9LocalBranchMaterialization" in content
        assert "approve-wave9-local-pr-plan-materialization" in content
        assert "materialize-wave9-local-pr-plan" in content
        assert "wave9-6-local-branch-materialization" in content
        assert "wave9-5-local-pr-plan" in content
        assert "local_branch_materialized" in content
        assert "local_verifier_passed" in content
        assert "provider_call_count `0`" in content or "provider_call_count 0" in content
        assert "local_branch_created: true" in content
        assert "branch_pushed: false" in content
        assert "external_pr_created: false" in content
        assert "merge_performed: false" in content
        assert "deploy_performed: false" in content
        assert "promotion_attempted: false" in content
    ...
    assert "approve-wave9-local-pr-plan-materialization" in cli_reference
    assert "materialize-wave9-local-pr-plan" in cli_reference



def test_status_docs_capture_wave9_7_local_branch_commit() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "Wave9-7" in content
        assert "Wave9LocalBranchCommit" in content
        assert "approve-wave9-local-branch-commit" in content
        assert "commit-wave9-local-branch" in content
        assert "wave9-7-local-branch-commit" in content
        assert "wave9-6-local-branch-materialization" in content
        assert "local_branch_commit_created" in content
        assert "local_verifier_passed" in content
        assert "provider_call_count `0`" in content or "provider_call_count 0" in content
        assert "local_commit_created: true" in content
        assert "branch_pushed: false" in content
        assert "external_pr_created: false" in content
        assert "merge_performed: false" in content
        assert "deploy_performed: false" in content
    ...
    assert "approve-wave9-local-branch-commit" in cli_reference
    assert "commit-wave9-local-branch" in cli_reference



def test_status_docs_capture_wave9_8_draft_pr_gate() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "Wave9-8" in content
        assert "Wave9DraftPRApproval" in content
        assert "Wave9DraftPR" in content
        assert "approve-wave9-draft-pr" in content
        assert "create-wave9-draft-pr" in content
        assert "auto_merge_enabled: false" in content
        assert "merge_performed: false" in content
        assert "deploy_performed: false" in content
        assert "production_mutated `false`" in content or "production_mutated: false" in content

    assert "approve-wave9-draft-pr" in cli_reference
    assert "create-wave9-draft-pr" in cli_reference



def test_status_docs_capture_wave9_9_10_review_ingestion() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    cli_reference = CLI_REFERENCE.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "Wave9-9/10" in content
        assert "Wave9PRSemanticReview" in content
        assert "Wave9CapabilityIngestion" in content
        assert "wave9-pr-semantic-review" in content
        assert "wave9-capability-ingest" in content
        assert "provider_call_count `0`" in content or "provider_call_count 0" in content
        assert "mutates_state: false" in content
        assert "routing_table_mutated: false" in content
        assert "merge_performed: false" in content
        assert "deploy_performed: false" in content
        assert "production_mutated `false`" in content or "production_mutated: false" in content

    assert "wave9-pr-semantic-review" in cli_reference
    assert "wave9-capability-ingest" in cli_reference



def test_status_docs_capture_wave9_11_15_merge_handoff() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    for content in (readme, outline):
        assert "Wave9-11/15" in content
        assert "PR #4" in content
        assert "wave9-15-pr4-post-merge-no-deploy-handoff" in content
        assert "78ae64c6751741b9a0924fd8ff8457ba54c690ee" in content
        assert "695 passed" in content
        assert "deploy_performed: false" in content
        assert "production_mutated `false`" in content or "production_mutated: false" in content



def test_status_docs_capture_wave10_1_to_wave13_1_execution_batch() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    release_checklist = RELEASE_CHECKLIST.read_text(encoding="utf-8")

    for content in (readme, outline):
        assert "Wave10-1 through Wave13-1" in content
        assert "wave10-1-longitudinal-mini-program-20260615" in content
        assert "wave10-2-asset-reuse-smoke-20260615" in content
        assert "teacher_call_rate_delta: -1.0" in content
        assert "wave11-2-evidence-explorer-20260615" in content
        assert "wave12-1-creative-feedback-20260615" in content
        assert "wave13-1-deploy-rollback-readiness-20260615" in content
        assert "approval_applies: true" in content
        assert "deploy_performed: false" in content
        assert "production_mutated: false" in content

    assert "Wave13-1 deploy/rollback readiness" in release_checklist
    assert "wave13-1-deploy-rollback-readiness-20260615" in release_checklist



def test_status_docs_capture_wave14_closure_batch() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    release_checklist = RELEASE_CHECKLIST.read_text(encoding="utf-8")

    for content in (readme, outline, release_checklist):
        assert "Wave14-1 through Wave14-4" in content
        assert "wave14-closure-20260615" in content
        assert "Wave14ClosureRunner" in content
        assert "longitudinal_task_count: 3" in content
        assert "operator_cockpit_v1_ready: true" in content
        assert "routing_learning_apply_loop_ready: true" in content
        assert "deploy_target_selected: local-release-artifact" in content
        assert "requires_exact_deploy_approval: true" in content
        assert "deploy_performed: false" in content
        assert "production_mutated: false" in content


def test_status_docs_capture_wave14_real_environment_authorized_run() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    release_checklist = RELEASE_CHECKLIST.read_text(encoding="utf-8")

    for content in (readme, outline, release_checklist):
        assert "Wave14 real-environment 1-4" in content
        assert "wave14-real-env-1to4-20260615" in content
        assert "feiyue-mid-deepseek-pro" in content
        assert "provider_call_count: 1" in content
        assert "static_export_all_ok: true" in content
        assert "safe routing apply hold" in content
        assert "steady-4c" in content
        assert "deploy_performed: false" in content
        assert "production_mutated: false" in content


def test_status_docs_capture_wave14_filtered_routing_apply() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    release_checklist = RELEASE_CHECKLIST.read_text(encoding="utf-8")

    for content in (readme, outline, release_checklist):
        assert "Wave14 filtered Routing Apply" in content
        assert "wave14-real-env-1to4-20260615-routing-learning-filtered" in content
        assert "applied_profiles: feiyue-mid-deepseek-pro" in content
...
def test_status_docs_capture_wave14_deployment_2_public_pages_repo() -> None:
    readme = README.read_text(encoding="utf-8")
    outline = OUTLINE.read_text(encoding="utf-8")
    release_checklist = RELEASE_CHECKLIST.read_text(encoding="utf-8")

    for content in (readme, outline, release_checklist):
        assert "Wave14 Deployment 2 GitHub Pages Static Cockpit" in content
        assert "Feiyue-operator-cockpit" in content
        assert "https://sinonchum.github.io/Feiyue-operator-cockpit/wave14-real-env-20260615/" in content
        assert "public_pages_commit: 964c138914c567d1b0f0102597df1bd23e3a28d4" in content
        assert "github_pages_config_mutated: true" in content
        assert "private_source_repo_publicized: false" in content
        assert "external_api_deployed: false" in content
        assert "provider_call_count: 0" in content
        assert "production_mutated: false" in content
